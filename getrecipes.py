...
def getrecipes(cuisine,category,time,rating,ingredients):
    ...
    if cuisine == 'Any':
        df = recipes.query('time <= @time and cuisine in @cuisines and category == @category and rating >= @rating')
    if category == 'Any':
        df = recipes.query('time <= @time and cuisine == @cuisine and category in @categories and rating >= @rating')
    if cuisine == 'Any' and category == 'Any':
        df = recipes.query('time <= @time and cuisine in @cuisines and category in @categories and rating >= @rating')
    if cuisine != 'Any' and category != 'Any':
        df = recipes.query('time <= @time and cuisine == @cuisine and category == @category and rating >= @rating')
    df_ing = df.copy()
